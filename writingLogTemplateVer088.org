...
# Note that the actual title of the paper is in the preamble drawer.
...
#+LATEX:\title{Polytglot crystallographic computing in Org-mode}
...
*** Guidance on central question of the paper                         :noexport:
:GUIDANCE:
The central question our hypothesis that is to be tested as the core idea of the paper.
Every part of the paper should be built around the central question.
Everything else is extra and potentially dispensable.
Getting the wording right for the central question requires testing out alternate wordings in the form of sentences.
It usually takes about a half dozen trial sentences to find one that articulates the question you wish to address.
Subsequent development of the introduction can lead to refinement of the central question.
:END:
# field for my lab?
...
*** Guidance on potential target journals for submission             :noexport:
:GUIDANCE:
...
** Acknowledgements
:PROPERTIES:
:CUSTOM_ID: subsec:acknowledgements
:END:
...
#+transclude: [[file:/Users/blaine/6112MooersLabGitHubLabRepos/writingLogTemplateInOrg/AIuse.org]]
...
:END:
#+LaTeX:\index{data}
*** Guidance on Data                                   :noexport:
:GUDIANCE:
This section catalogs the data to be used in the paper.
:END:

** Data management plan                                 :noexport:
:GUDIANCE:
You might start this section with the data management plan included in the supporting grant application.
Then you might include updates to this plan as the project evolves.
:END:
...
#+CAPTION: Nodes in use in this project.
#+ATTR_LATEX: :booktabs t :center t :align >{\raggedright\arraybackslash}p{1.4cm} >{\raggedright\arraybackslash}p{3.6cm}  >{\raggedright\arraybackslash}p{3.6cm} >{\raggedright\arraybackslash}p{1.5cm}
| Folgezettel | Node title | Relevance to project| Date of last entry|
|-----+----+---+---|
| 30.5 | Restrart plan | central | 2026-05-23 |
|-----+----+---+---|
...
** 2025-03-20
#+Latex:\index{2025-03-20}
# Enter M-RET to create a new entry
# Snippet entry has goals and accomplishments
# You want to include correspondence in quote block(s).

*** Accomplishments
- Submitted routing form to or center track.
- Drafted the summary statement.
- Assigned project number and created folder for project.




  ** 2025-03-21
#+Latex:\index{2025-03-21}
# Enter M-RET to create a new entry
# Snippet entry has goals and accomplishments
# You want to include correspondence in quote block(s).

*** Bridge written at end of last writing session.
Copy and paste from section below if you want to keep a record of these thoughts.

**** Actions to be taken in the next writing session.
-

**** Summarize current state: What are the biggest challenges, open questions, worries, or future roadblocks?
-

**** Record the vital details that may fade away before the next writing session.
-

**** What is you intention for the next writing session?
...
** Bridge to next session                                            :restart-here:
:PROPERTIES:
:CUSTOM_ID: subsec:next
:END:
#+LaTeX:\index{next action}
*** Guidance on bridge to next session                                :noexport:
:GUIDANCE:
The idea is not write until the generative well is completely dry.
Leave some material behind so that you can reengage more easily.
This idea is from Ernest Hemingway.

If you paint a clear enough picture, it will be easier to re-engage.
The four questions are from Tiago Forte (2022) 'Building a Second Brain'.

You could copy this subtree and paste into the daily entry to have the
answers to these questions in front of you. They might be a useful
record.

- List one or more actions to do in the next session.
- Summarize current state: What are the biggest challenges, open questions, worries, or future roadblocks?
- Record the vital details that may fade away before the next writing session.
- What is you intention for the next writing session?
...
** To be done or could be done
:PROPERTIES:
:CUSTOM_ID: subsec:to-do
:END:
#+LaTeX:\index{To be done}
*** Guidance on to be done                               :noexport:appendtodos:
:GUIDANCE:
...
I use an unordered list below.

I leave this list unprioritized.
I pull out a subset of TODOs to work on in a given session.
I do the priotizing of this subset at the start of the writing session.

Avoding adding deadlines to your TODOs else your org-agenda will become clogged with unfnished TODOs.

This log file can be included in the list of org files scanned for deadlines and scheduled events by org-agenda.

The package mooerslab/org-10k-projects eases capturing TODOs in this file via the org-capture template for projects.
:END:
...
** Protocol for BibTeX to BibLaTeX library conversion
...
#+LaTeX:\item Start R-Studio
#+LaTeX:\item Run the following code after editing the file paths:
#+LaTeX:\end{enumerate}
#+LaTeX:
#+LaTeX:\begin{minted}{R}
#+LaTeX:library(devtools)
#+LaTeX:install_github("GeoBosh/rbibutils")
#+LaTeX:library(rbibutils)
#+LaTeX:bibConvert("/Users/blaine/Documents/global.bib", "/Users/blaine/Documents/globalBiblatex.bib", "bibtex", "biblatex")
#+LaTeX:\end{minted}
